    ...
    >
      <div>
        <Input
          ...
          name="email"
          placeholder="이메일 입력"
          className="h-[40px] w-full rounded-t-md border border-b-0 border-[#7A7A7A] indent-2 text-black"
        />
        {errors.email && (
      ...
      <div>
        <Input
          ...
          name="password"
          placeholder="비밀번호 입력"
          className="h-[40px] w-full rounded-b-md border border-t-0 border-[#7A7A7A] indent-2 text-black"
        />
        {errors.password && (
          <p className="text-sm text-red-500">{errors.password.message}</p>
        )}
      </div>
      <p className="mt-2 text-sm text-[#A7A7A7]">비밀번호를 잃어버렸어요?</p>
    </SignupForm>
  );
}
